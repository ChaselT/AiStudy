...
4. **代码全进 Git（三仓库体系，2026-07-28 拆分）**：
   - `ChaselT/AiStudy`（私有，**将来可公开**作学习轨迹证明）：学习代码，本目录。**2026-07-28 已预清理**：练习中真实姓名已脱敏（张三→张三等）、PLAN.md 已移出至私有仓库。公开前仍需：①CLAUDE.md 个人画像斟酌脱敏或移出；②**git 历史中仍有旧的个人信息（如 3d5c7dc 等旧提交含真实姓名）——公开时用 `git filter-repo` 重写历史批量替换（保留提交轨迹/日期，这是学习轨迹的价值所在），绝不能直接翻公开开关**
