...
1. **学习文档由 Claude 生成**，不让学员上网找课。每进入新阶段，在 Obsidian `AI学习\` 对应目录生成该阶段文档：每篇一个概念、frontmatter tags（`AI学习` + `阶段N` + 主题）、Java 类比、可运行代码示例、动手任务（指向对应 phase 目录）、`[[双链]]`关联；每阶段一篇 MOC。**每篇「相关笔记」开头必须有导航条**（`本阶段第 N/M 课 | 上一篇 | 下一篇`），学员读完能直接跳下一课，不用回 MOC 查表；**动手任务若依赖后面课程的知识，必须标注前置依赖**（阶段 1 的 ex02_usage_watch 踩过：题目要多轮对话，但 messages 数组下一课才讲）。深度根据学员上一阶段的掌握情况调整。**生成文档的同时，把每篇的动手任务预建为作业骨架文件**（对应 phase 目录，题目写成文件头 docstring，含完成标准），学员打开文件即可开写（2026-07-28 起约定）。
...
3. **进度记录**：帮学员更新 `学习进度.md` 的勾选和周记；每 4 周提醒复盘；进度允许 ±2 周浮动。
   - **知识库 lint（2026-07-31 采纳自 Karpathy 的 LLM Wiki 模式）**：每 4 周月度复盘时，对 `AI学习\` 做一次体检——①**过期断言**（模型名/SDK 写法/价格/版本基线是否已变，必要时联网核实）；②**矛盾**（不同笔记对同一问题说法冲突）；③**断链与孤儿页**（`[[双链]]` 指向不存在的笔记、没有任何入链的页面）；④**未回填的实践结论**（学员踩过的坑是否已进错题本、是否已更新相关笔记）。发现即修，并在 `学习进度.md` 记一行 lint 结果。**理由**：AI 生成的知识库首要失效模式是 drift 而非缺失——阶段 1 文档生成 5 天后就有多处模型名过时（deepseek-chat 退役），不主动巡检就会拿着过期知识去实战。
   - **开课前预检（2026-08-16 学员要求，每开一节新课必做）**：学员开始学某一课**之前**，先花几分钟核这一课的笔记，**不用等月度 lint**。查三类：
     ①**外部漂移**——笔记里写死的模型名/版本号/体积/榜单排名/`ollama pull` 的名字是否仍然成立（联网核，别凭记忆）；
     ②**我自己的算错**——笔记里凡是有算术、预算、容量估算的地方重算一遍，**用学员自己实测过的数字**而不是理论值；
     ③**与既有实测结论矛盾**——笔记的建议是否和错题本 / 前面阶段的实测数据打架。
     发现即改，并在回复里明说「改了什么、为什么」，不要静默替换。
     **理由**：这条不是为了防"过时"——2026-08-16 那次预检查出的是**我在阶段 2 MOC 里把三个模型的显存加错了**（写成"可以和 27b 同时驻留"，实际只剩 1.9GB 给 KV cache）。外部漂移和我自己写错，对学员是同一种伤害：**拿着错的前提去实践，且不知道该怀疑教材**。而学员的时间是挤出来的，一次错误指令的代价是一整晚。
...
10. **实战资产 video-clipper-pro（2026-07-31 纳入）**：学员另有一款自己设计、AI 代写实现的商业剪辑软件（`E:\workspace\python\video-clipper-pro`，src 28k 行 / 967 测试 / 含手写 MCP Server 与多 provider LLM 网关），**但他未细读代码**（懂 why 不懂 how）。详见 Obsidian `00-总览\实战资产-video-clipper-pro.md`。约定：
    ...
    - **eval 体系是最高优先级短板**（项目零评测，LLM 自评分无 ground truth）；已并入项目①一起练
    - 阶段 2/4 的优化（历史爆款 RAG、YOLO 服装检测、QLoRA 初筛降本、本地 Whisper）是真护城河——把私有数据变成别人复刻不了的能力
11. **转行加速机制**（详见 `D:\ObsidianVault\PLAN-AI转行学习计划.md` 同名章节）：在职落地 AI、博客边学边写（每阶段 1-2 篇）、阶段 2 起每周 2-3 道算法题（Python）、第 24 周起试水投递、JD 校准（阶段 3 后启动）。
